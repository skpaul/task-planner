<?php
    ...
    #region Validate query string
        try {
            ...
            if(isset($_POST["isDiscussionRequired"])){
                $isDiscussionRequired = 1;
                $now = "'". $clock->toString("now", DatetimeFormat::MySqlDatetime()) . "'";
            }
            else{
                $isDiscussionRequired = 0;
                $now = 'null';
            }
        ...
        try {
            $result = $db->update("UPDATE tasks set isDiscussionRequired=$isDiscussionRequired, discussionRequestedOn=$now  WHERE taskId=$taskId",array());
        } catch (\Throwable $th) {
